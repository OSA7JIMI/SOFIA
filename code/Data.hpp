...
private:
  ...
  vector<vector<bool> > relnodehasarc_original_;      // [relation][entity] = true if arc is induced
  vector<vector<bool> > relnodehasinvarc_original_;   // [relation][entity] = true if arc is induced
  Query query_;
  ...
  vector<double> relationFrequencyBias_;
...
public:
  ...
  pair<int,int> getRandomPair(int relationId);
  ...
  // Clear training graph while keeping relations intact (for inductive prediction)
  void clearGraphKeepRules();
  
  // Load test data and build test graph (called after clearGraphKeepRules)
  void readDataForTesting(Parameters& params, string dataFile, string forwardChainFileName);

  void addRelationFrequencyBias(vector<double>& relationFrequencyBias) {relationFrequencyBias_ = relationFrequencyBias;}
  vector<double>& getRelationFrequencyBias() { return relationFrequencyBias_; }
  void computeRelationFrequencies();

  void getEntitiesOfInterest(int relationId, int whichCombination, map<int,set<int> >& rEntities, map<int,set<int> >& lEntities);

  bool hasRelaxedPath(Rule& rule, pair<int,int>& pair);
